       SUBROUTINE PREPROC_KWA(NETWORK_ID,FILELIST,BYEAR,IVERBOSE)

C***********************************************************************
C*    Preprocess Royal Marshall Islands or Kwajalein,LBA rain gauge network
C***********************************************************************

      INCLUDE 'preproc.par'
C***  MODULE FOR PREPROCESSING KWAJALEIN RAW DATA
      ...
      INTEGER*4 IBYR,YR,MON,DAY,JDAY,HR,MIN,SEC,FLAG_YR, HROFF
      DOUBLE PRECISION T
...
c      print*,MON,DAY,YR,HR,MIN,SEC,RAIN
 15   FORMAT(6(I2,1X),1X,F4.2)

       IF(FLAG_YR.EQ.0)THEN
          WRITE(CYR,8)YR   
          IF(YR.LT.IBYR.OR.YR.GT.(IBYR+1))THEN
...
C***  10 SECONDS. MULTIPLE TIPS ARE DISTRIBUTED ACROSS EVEN TIME INTERVALS OVER 
C***  A 10 SECOND PERIOD
      
      JDAY = JULDAY(YR,MON,DAY)
      NTIP = RAIN*100           ! # TIPS PER RECORD
      IF(NTIP.GT.1)THEN
         IF(NETWORK_ID.EQ.'LBA')THEN 
            IF(NTIP.GT.2)GOTO 5  !QC DATA-IGNORE MULTIPLE TIPS > 2 OVER 5 SEC
            DTIP = 5./NTIP
         ELSE
            DTIP = 10./NTIP     ! TIME INTERVAL BETWEEN TIPS I < 10S
         ENDIF
         CURSECOND_OF_MIN = SEC
         DO I =1,NTIP
            ISEC = DTIP*(I -.5)+.5
         IF(NETWORK_ID.EQ.'LBA')THEN 
            SEC =   CURSECOND_OF_MIN - 5 + ISEC+.5
         ELSE
            SEC =   CURSECOND_OF_MIN - 10 + ISEC+.5
         ENDIF
